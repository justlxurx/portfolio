class ManageNftApi {
  ...
  async createNft(num: number, img: FormData) {
    const res = await fetch(`${this.url}/property/${num}/nft/create`, {
      method: "POST",
      headers: {
        "x-api-key": `admin`,
      },
  ...
  async mintNft(num: number) {
    const res = await fetch(`${this.url}/property/${num}/nft/mint`, {
      method: "POST",
      headers: {
  ...
  async getNft(num: number) {
    const res = await fetch(`${this.url}/property/${num}/nft`, {
      method: "GET",
    });
    if (!res.ok) {
      const errorText = await res.text();
